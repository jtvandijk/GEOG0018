...
Let us focus on the `lsoa_emp` dataset in first instance. We will start by inspecting the dataset:
...
### Data manipulation
Since the `lsoa_emp` dataset was extracted using the [Custom Dataset Tool](https://www.ons.gov.uk/datasets/create), we need to convert the data once more from long to wide format. We can do this again by pivoting each of the tables:
...
When we want to compare more than two groups, we can use the Analysis of Variance (ANOVA) test. ANOVA evaluates whether there are any statistically significant differences between the means of multiple groups. If the assumptions of ANOVA are violated, we can use the Kruskal-Wallis test, which is a non-parametric method that compares the ranks across the groups.
...
If we want to compare more than two, we can use an [Analysis of Variance (ANOVA) test](https://en.wikipedia.org/wiki/Analysis_of_variance). An ANOVA is a statistical method used to compare the means of three or more groups to determine if at least one group mean significantly differs from the others. For instance, if we extend our analysis from Camden and Sutton to include Hammersmith and Fulham, we can test for differences in means among these three areas.
...
We can do this by filtering our LSOA dataframe to include only those LSOAs that fall within Camden, Sutton, and Hammersmith and Fulham. Next, we need to extract the borough name from the `lower_layer_super_output_areas` column. We can accomplish this by creating a new variable that excludes the last five characters of the `lower_layer_super_output_areas` variable. 
...
The ANOVA output shows the F-statistic with a value 75.31, with a $p$-value `< 0.001`. We can conclude that there are significant differences in unemployment rates across the boroughs. This means we reject the null hypothesis that all group means are equal. 
...
1. Use an appropriate statistical test to determine whether the average proportion of the employed population differs between LSOAs in *Bexley* and *Harrow*.
2. Apply the relevant test to compare the proportion of the proportion of people *aged 50 years and over* across the boroughs of Lambeth, Southwark, and Westminster.
3. Include the test outputs in the appendix of your assignment and provide a brief interpretation of the results in a few sentences.

## Before you leave
This week, we explored how to statistically compare different groups to assess whether they are different, using both parametric and non-parametric methods. Next week, we'll shift our focus from group comparisons to analysing associations and relationships between variables. Looking forward? For now, take [some time to unwind](https://www.youtube.com/watch?v=KMxucpLOF5s) and get ready for what's next!