...
1. Open a new Excel spreadsheet. 
2. From the `LSOA2021-population.xlsx` spreadsheet, cut (**Edit** -> **Cut**) all cells from columns **A to B** and rows **19,790 to 2,4783** and paste these into this new spreadsheet.
...
::: {.callout-warning}
While the map above is acceptable for today, it is technically incorrect because it displays absolute numbers on a choropleth. This practice should never be used unless the spatial units are identical in size, as larger areas will draw disproportionate attention and skew the visualisation.
:::
